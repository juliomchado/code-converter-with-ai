...
export class ApiValidationService {
  ...
  static async validateGeminiKey(apiKey: string): Promise<ValidationResult> {
    ...
    try {
      ...
      if (response.ok) {
        ...
        return { isValid: false, error: 'API key is invalid or has no permissions' };
      } else {
  ...
  static async validateOpenRouterKey(apiKey: string): Promise<ValidationResult> {
    if (!apiKey || !apiKey.trim()) {
      return { isValid: false, error: 'API key is required' };
    }
  ...
  static async validateClaudeKey(apiKey: string): Promise<ValidationResult> {
    ...
    if (!apiKey.startsWith('sk-ant-')) {
      return { isValid: false, error: 'Invalid Claude API key format (should start with sk-ant-)' };
    }
    ...
    return {
      ...
      providerInfo: {
        name: 'Claude',
        quotaInfo: 'Key format is valid (verification requires actual API call)'
      }
    };
  }
