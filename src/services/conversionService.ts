...
export class ConversionService {
  ...
  private static readonly DEEPSEEK_ENDPOINT = 'https://api.deepseek.com/v1/chat/completions';
  ...
  private static readonly GEMINI_ENDPOINT = 'https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash-latest:generateContent';
  ...
  static async convertCode(
    ...
    userKeys: { deepseek?: string; claude?: string; openai?: string; openrouter?: string; gemini?: string }
    ...
    return {
      ...mockResult,
      error: mockResult.error || '⚠️ No valid API keys provided. Please add your API key in the settings above. Google Gemini offers a generous free tier!'
    };
  }

  // OpenRouter API (can access DeepSeek and other models)
  static async convertWithOpenRouter(request: ConversionRequest, apiKey: string): Promise<ConversionResponse> {
    try {
...
${request.sourceCode}`;
      ...
      if (error instanceof Error) {
        if (error.message.includes('401')) {
          ...
          errorMessage = 'OpenRouter: Insufficient credits. Please add credits to your account.';
  ...
  static async convertWithGemini(request: ConversionRequest, apiKey: string): Promise<ConversionResponse> {
    try {
      const prompt = `Convert the following ${request.fromLanguage} code to ${request.toLanguage}. 
Only return the converted code without any explanations, comments, or markdown formatting. 
Return just the raw code that can be copied and used directly.
...
${request.sourceCode}`;
      ...
      console.log('Gemini API URL:', `${this.GEMINI_ENDPOINT}?key=${apiKey.substring(0, 10)}...`);
      ...
      if (error instanceof Error) {
        if (error.message.includes('401')) {
          ...
          errorMessage = 'Gemini API key does not have permission or quota exceeded';
        } else if (error.message.includes('429')) {
  ...
  static async convertWithDeepSeek(request: ConversionRequest, apiKey: string): Promise<ConversionResponse> {
    try {
      const prompt = `Convert the following ${request.fromLanguage} code to ${request.toLanguage}. 
Only return the converted code without any explanations, comments, or markdown formatting. 
Return just the raw code that can be copied and used directly.
...
${request.sourceCode}`;
      ...
      return {
        convertedCode: '',
        error: error instanceof Error ? error.message : 'DeepSeek conversion failed'
      };
    }
  }

  // Claude API
  static async convertWithClaude(request: ConversionRequest, apiKey: string): Promise<ConversionResponse> {
    try {
      const prompt = `Convert the following ${request.fromLanguage} code to ${request.toLanguage}. 
...
\`\`\``;
      ...
      return {
        convertedCode: '',
        error: error instanceof Error ? error.message : 'Claude conversion failed'
      };
    }
  }

  // OpenAI API
  static async convertWithOpenAI(request: ConversionRequest, apiKey: string): Promise<ConversionResponse> {
    try {
      const prompt = `Convert the following ${request.fromLanguage} code to ${request.toLanguage}. 
Only return the converted code without any explanations, comments, or markdown formatting. 
Return just the raw code that can be copied and used directly.
...
${request.sourceCode}`;
      ...
      return {
        convertedCode: '',
        error: error instanceof Error ? error.message : 'OpenAI conversion failed'
      };
    }
  }

  static async mockConvertCode(request: ConversionRequest): Promise<ConversionResponse> {
...
print(fibonacci(10))`,
    ...
    const convertedCode = mockConversions[key] || `// MOCK CONVERSION - This is not a real AI conversion
